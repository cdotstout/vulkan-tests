...
class VkCopyTest {
 public:
  ...
  bool Initialize();
  bool Exec(bool wait_and_signal);
  bool Validate();
...
bool VkCopyTest::Initialize() {
  ...
  {
    auto sem_create_info = vk::SemaphoreCreateInfo();
    auto rv = ctx_->device()->createSemaphoreUnique(sem_create_info);
    if (rv.result != vk::Result::eSuccess) {
      RTN_MSG(false, "VK Error: 0x%x - vk::Queue submit failed.\n", rv.result);
    }
    semaphore_ = std::move(rv.value);
  }
...
bool VkCopyTest::Exec(bool wait_and_signal) {
  ...
  printf("Submitting command buffer: wait_and_signal %d\n", wait_and_signal);
